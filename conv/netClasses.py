...
class convEP(nn.Module):
    def __init__(self, args):
        ...
        self.adj_acom_step = args.adj_acom_step        
        self.coe           = args.coe            
    ...
    def stepper(self, data, s, inds, target = None, beta = 0, s_free = None, coe= None, return_derivatives = False, inplace = False): 
        ...
        if not inplace:                  
            for i in range(len(s)):
              ## adjusted adaptation or not
              if s_free is None:
                s[i] = s[i] + dsdt[i]
              else:
                s[i] = (1 - coe)*(s[i] + dsdt[i]) + coe*s_free[i]
        else:
            for i in range(len(s)):
                s[i] += dsdt[i]            
    ...
    def forward(self, data, s, inds, pred_inp_size=[12], seq = None, indseq = None, method = 'nograd', beta = 0, target = None, s_free=None, **kwargs):
        T             = self.T
        Kmax          = self.Kmax
        adj_acom_step = self.adj_acom_step   
        coe           = self.coe       
        if len(kwargs) > 0:
            K = kwargs['K']
        else:
        ...
        if (method == 'withgrad'):                
            ...
            if beta == 0:
              ...
              for t in range(T):                  
                ...
                all_clamped_steps = Kmax + adj_acom_step           
                for t in range(all_clamped_steps):  
                  ## When Kmax < t,  adjusted adaptation starts
                  if (t < Kmax): 
                    s, inds = self.stepper(data, s, inds, target, beta)
                  else:
                    s, inds = self.stepper(data, s, inds, target, beta, s_free = s_free, coe = coe)
    ...
    def computeGradients(self, beta, data, s, inds, seq, indseq):
        ...
        for i in range(self.nconv - 1):
            ...
            gradconv.append((1/(beta*batch_size))*(F.conv2d(s[self.nc + i + 1].permute(1, 0, 2, 3), self.unpool(s[self.nc + i], inds[self.nc + i], output_size = output_size).permute(1, 0, 2, 3), padding = self.P)
                            - F.conv2d(s[self.nc + i + 1].permute(1, 0, 2, 3), self.unpool(seq[self.nc + i], indseq[self.nc + i], output_size = output_size).permute(1, 0, 2, 3), padding = self.P)).permute(1, 0, 2, 3))
    ...
    def updateWeights(self, beta, data, s, inds, seq, indseq, epoch):
