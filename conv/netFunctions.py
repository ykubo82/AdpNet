        ...
        for batch_idx, (data, targets) in enumerate(train_loader):
            ...
            if method == 'BPTT':  
                ...
                with torch.no_grad():
                    ...
                    # clamped phase  
                    s, inds, _  = net.forward(data, s_with_delay, inds, [0], seq, indseq, s_free = s_free, target = targets, beta = net.beta, method = 'nograd')                                                            
                     
                    net.updateWeights(net.beta, data, s, inds, seq, indseq, epoch) 
                    #########################################################################################
